...
%DEMc = imposemin(Strunk,DEM)
%k = ksn(S,DEMc,A,.45, 200);
%g=gradient(Strunk, DEMc );
figure
plotdz(Strunk, DEM, 'color', k, 'colormethod'  , 'surface')
colormap(jet)
colorbar
caxis([0, 85])
%% CURVATURE
...
find_tribbot=find(ismember(inan_tot, inan))-1;
inan_top=inan_tot(find_tribbot);
...
% 
% end
snapx_trib=NaN(length(inan), 1);
snapy_trib=NaN(length(inan), 1);
i_ms=NaN(length(inan), 1);
elev_threshold=2;
for i=1:length(inan)
    itribtot=(inan_top(i)+1):(inan(i)-1);
    elev_tribtot=elev(itribtot);
    itribtot=itribtot(elev_tribtot>elev_tribtot(end)+elev_threshold); %eliminating the part of the trib that is less than 0.5 m above the outlet to get rid fo the influence of the floodplain
    if length(itribtot)>5
        [snapx, snapy]=snap2stream(strunky, lat(itribtot(end)), long(itribtot(end)));
        snapx_trib(i, 1)=snapx;
        snapy_trib(i, 1)=snapy;
        ix=find(abs(tlat-snapx)<=1 & abs(tlong-snapy)<=1);
        if length(ix)>1
           distix= sqrt((tlat(ix)-snapx).^2+(tlong(ix)-snapy).^2);
           mindistix=find(distix==min(distix));
           ix=ix(mindistix);
        end
        if length(ix)==1
...
for i=1:length(inan)
    ...
    if ~isnan(i_ms(i)) & length(itribtot)>5
        %for the trib first whats the angle

        if length(itribtot)>30%sum(isnan(smoothx((inan(i)-1):-1:(inan(i)-11))))==0
            poly1fit=polyfit(smoothx(itribtot), smoothy(itribtot), 1);%polyfit(lat(itribtot(end-30):itribtot(end)), long(itribtot(end-30):itribtot(end)), 1);%(smoothx((inan(i)-1):-1:(inan(i)-11), 1), smoothy((inan(i)-1):-1:(inan(i)-11)), 1);
        end
        if length(itribtot)>5 & length(itribtot)<11 %sum(isnan(lat(itribtot(end-11):itribtot(end))))~=0  % do a less robust polyfit for those very short tribs
            poly1fit=polyfit(smoothx(itribtot), smoothy(itribtot), 1);%polyfit(lat(itribtot(end-4):itribtot(end)), long(itribtot(end-4):itribtot(end)), 1);%polyfit(smoothx((inan(i)-1):-1:(inan(i)-3)), smoothy((inan(i)-1):-1:(inan(i)-3)), 1);
        end
        ...
        if i==1 | i==516
            poly1fit2=polyfit(tsmoothx((i_ms(i))+1:-1:(i_ms(i)-4)), tsmoothy((i_ms(i))+1:-1:(i_ms(i)-4)), 1);
        end
...
xbot=NaN(length(inan), 1);
ybot=NaN(length(inan), 1);
elev_threshold=2;
for i=1:length(inan)
    if ~isnan(i_ms(i)) & i~=516
        itribtot=(inan_top(i)+1):(inan(i)-1);
        elev_tribtot=elev(itribtot);
        itribtot=itribtot(elev_tribtot>elev_tribtot(end)+elev_threshold); %eliminating the part of the trib that is less than 0.5 m above the outlet to get rid fo the influence of the floodplain
        if ~isempty(itribtot)
            xbot(i)=long(itribtot(end));
            ybot(i)=lat(itribtot(end));
            if length(itribtot)>16
                ksn_tribbot(i)=mean(ks(itribtot(end)-16:itribtot(end)));%mean(ks(itribtot));%mean(ks(itribtot(end)-30:itribtot(end)));%
            end
            if length(itribtot)<17
                ksn_tribbot(i)=mean(ks(itribtot));
            end

    %         ksn_tribbot(i)=mean(ks(inan(i)-30:inan(i)-2)); %uncomment if new method fails
    %         if isnan(mean(ks(inan(i)-30 : inan(i)-2))) %uncomment if new method fails
    %             ksn_tribbot(i)=mean(ks(inan(i)-15 : inan(i)-2));%uncomment if new method fails
    %         end %uncomment if new method fails
    %         if isnan(mean(ks(inan(i)-15 : inan(i)-2)))
    %             ksn_tribbot(i)=mean(ks(inan(i)-5 : inan(i)-2));
    %         end
            da_trib(i)=area(itribtot(end));
            curve_ms(i)=mean(tcv((i_ms(i)-1):(i_ms(i)+1)));
...
for i=1:binnum
    ...
    if sum(binn)>10 %dont want to bin stuff with less than 5
...
postrib=fitlm(postbinc(~isnan(postbink)), postbink(~isnan(postbink)), 'poly1')
figure
tiledlayout(2, 1)
nexttile
...
negtrib=fitlm(negtbinc(~isnan(negtbink)), negtbink(~isnan(negtbink)), 'poly1')
nexttile
plot(curve_ms(a_tribtoms<0 ), ksn_tribbot(a_tribtoms<0), 'o', 'MarkerEdgeColor', [.5, .5, .5])
hold on
plot(negtrib)
...
xx=curve_ms(a_tribtoms>0 );
yy=ksn_tribbot(a_tribtoms>0);
postrib=fitlm(postbinc(~isnan(postbinc)), postbinda(~isnan(postbinc)), 'poly1')
figure
tiledlayout(2, 1)
nexttile
...
%xx2=curve_ms(a_tribtoms<0 );
%yy2=ksn_tribbot(a_tribtoms<0);
negtrib=fitlm(negtbinc(~isnan(negtbinc)), negtbinda(~isnan(negtbinc)), 'poly1')
nexttile
plot(curve_ms(a_tribtoms<0 ), da_trib(a_tribtoms<0), 'o', 'MarkerEdgeColor', [.5, .5, .5])
hold on
plot(negtrib)
...
%%
figure
plot(dist_tribbot(a_tribtoms<0), ksn_tribbot(a_tribtoms<0), 'ro')
hold on
plot(dist_tribbot(a_tribtoms>0), ksn_tribbot(a_tribtoms>0), 'ko')
...
scatter(snapx_trib, snapy_trib, 60, 'filled')
caxis([0, 80])
%colormap(jet)
%h = colorbar;
%h.Label.String = 'Angle (degrees)';
